...
if ( $(this).scrollTop() > 0 && $menu.hasClass("default") ){
	...
	$(".sandwich_catalog").click(function() {
		...
		if ($(".menu-dropdown").is(":hidden")) {
			...
			$(".sandwich_catalog").addClass("active");
		} else {
	...
	$('.item-dealer .link-page:not(.fancybox)').click(function(event) {
		...
		$(this).parent().parent().siblings().find(".tab-pane-dealer").fadeOut(0);
	...
	$(".item-nav__head").click(function() {
		...
		$(this).parent().siblings(".item-nav").removeClass("active");
		$(this).parent().siblings(".item-nav").find(".item-nav__content").slideUp(200);
	});
	...
	{
		if ($(window).width() < 992) { 
			$('.item-dealer .link-page').click(function() {
				$('html').animate({ 
					scrollTop: $(".tab-container-dealer").offset().top - 70
				}, 1000 
				);
			}); 
